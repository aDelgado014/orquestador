...
interface PixelAgentProps { agent: Agent }
const PixelAgent = ({ agent }: PixelAgentProps) => {
...
export function OrchestratorPage() {
  ...
  const [stats, setStats] = useState({ tasks: 0, tokens: 0, cost: 0 });
  ...
  const addLog = (agentId: string, message: string, type: ActivityLogEntry['type'] = 'info', tokens = 0, cost = 0) => {
    ...
    if (tokens > 0) {
      setStats(prev => ({ tasks: prev.tasks + 1, tokens: prev.tokens + tokens, cost: prev.cost + cost }));
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-lg font-bold text-zinc-100">AI Orchestrator</h2>
        <span className="px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/20 rounded text-[10px] font-mono text-emerald-500 animate-pulse">LIVE</span>
      </div>
      ...
      <div className="grid grid-cols-12 gap-6">
        <section className="col-span-12 lg:col-span-8 space-y-6">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
            <div className="p-4 border-b border-zinc-800 flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold uppercase tracking-wider">Virtual Office Floor</h3>
            </div>
            <div className="aspect-video bg-[#1a1a1a] relative overflow-hidden p-8 flex flex-col items-center justify-center">
              ...
              <div className="relative z-10 w-full max-w-2xl">
                ...
                <div className="grid grid-cols-3 gap-y-16 gap-x-8">
                  {agents.map(agent => (
                    <React.Fragment key={agent.id}><PixelAgent agent={agent} /></React.Fragment>
                  ))}
                </div>
              </div>
              <div className="absolute bottom-4 right-4 text-[10px] font-mono text-zinc-600">SECURE_ENCLAVE_V2.0</div>
            </div>
          </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                className="bg-emerald-500 hover:bg-emerald-400 disabled:bg-zinc-800 disabled:text-zinc-600 text-black font-bold px-6 py-3 rounded-lg flex items-center gap-2 transition-all"
              >
        ...
        <section className="col-span-12 lg:col-span-4 space-y-6">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl flex flex-col h-[500px]">
            <div className="p-4 border-b border-zinc-800 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Activity className="w-4 h-4 text-emerald-500" />
                <h3 className="text-sm font-bold uppercase">Activity Log</h3>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 font-mono text-[11px]">
              ...
              {logs.map((log) => (
                <motion.div
                  ...
                  key={log.id}
                  className="border-l-2 border-zinc-800 pl-3 py-1"
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className={`font-bold ${log.type === 'success' ? 'text-emerald-500' : log.type === 'error' ? 'text-red-500' : 'text-zinc-400'}`}>
                      [{log.role}] {log.agentName}
                    </span>
                    <span className="text-[9px] text-zinc-600">{log.timestamp.toLocaleTimeString([], { hour12: false })}</span>
                  </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
              ...
              <div className="w-full bg-zinc-800 h-1 mt-2 rounded-full overflow-hidden">
                <motion.div animate={{ width: isProcessing ? '84%' : '2%' }} className="bg-emerald-500 h-full" />
              </div>
            </div>
          </div>
        </section>
